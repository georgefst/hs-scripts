...
import Data.Csv.Parser.Megaparsec qualified
import Data.Csv.Parser.Megaparsec qualified as Csv
import Data.Either.Extra (maybeToEither)
import Data.Fixed (E2, E6, showFixed)
import Data.Foldable
import Data.Functor
import Data.List.Extra (List, enumerate, isPrefixOf)
...
import Text.Pretty.Simple (pPrintForceColor, pShow)
...
cmdInner projects model = \case
    ...
    Save -> unlessTimerRunning do
        -- TODO currently designed to be run once daily before exiting
        -- we should either make it mor flexible, or just actually run on exit (which `repline` makes easy)
        -- also, bear in mind especially that I often work past midnight - use start day not end
  ...
  where
    ...
    showActiveTimer t0 = do
        ...
        now <- getCurrentTime
        showTimeAndEarnings $ diffUTCTime now t0
...
main = do
    ...
    flip
        ...
        $ evalRepl
            ...
            (const $ pure "💰£⏲> ") -- TODO something more sensible, and ideally highlighted in my colourr
            (cmd . T.strip . T.pack)
            []
            Nothing
            Nothing
...
projectsPath = [osp|/syncthing/docs/work/contracting/projects.csv|]
projectsPathBase :: OsPath
projectsPathBase = [osp|/syncthing/docs/work/contracting/|]
