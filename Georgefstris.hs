...
app = let initialGridModel = let (p, random0) = uniform @Piece $ mkStdGen opts.seed; (next, random) = uniform @Piece random0; current = newPiece p in Model{pile = Grid $ A.replicate A.Seq (A.Sz2 opts.gridWidth opts.gridHeight) Unoccupied, current, next, ticks = 0, level = opts.startLevel, random, gameOver = False} in (component () (traverse_ $ publish keysPressedTopic) (\() -> div_ [] [div_ [id_ "grid"] +> grid initialGridModel, div_ [id_ "sidebar"] +> (\initialModel -> (component initialModel (either (\start -> when start $ subscribe' nextPieceTopic $ bimap (const False) Left) (either (modify . first . const) \b -> gets snd >>= \l -> let l' = bool (max opts.startLevel . pred) (min opts.topLevel . succ) b l in modify (second $ const l') >> publish setLevelTopic l')) (\(piece, level) -> div_ [] [div_ [class_ "next"] [let ps = shape piece; vMin = V2 (NE.minimum $ (^. lensVL _x) <$> ps) (NE.minimum $ (^. lensVL _y) <$> ps); vmax = V2 (NE.maximum $ (^. lensVL _x) <$> ps) (NE.maximum $ (^. lensVL _y) <$> ps); V2 w h = vmax - vMin + 1 in gridCanvas w h [] \f -> for_ ((- vMin) <$> ps) $ f piece], div_ [class_ "level"] [button_ [onClick $ Right $ Right False] [text "-"], div_ [] [text $ ms level], button_ [onClick $ Right $ Right True] [text "+"]]])){initialAction = Just $ Left True}) (initialGridModel.next, initialGridModel.level)])){subs = [keyboardSub $ mapMaybe opts.keymap . toList]}
