    ...
    Miso.embed $
        toComponent
            ( EnvCons
                ...
                -- TODO what's the concurrency story here
                    . Miso.sample
                    $ Miso.Component
...
topComponent :: forall env. Component "top" Word () env
topComponent =
    Component
        { initialState = 0
        , view = \m ->
            Miso.div_
                ...
                , embed EnvNil $ sub @'[ '("top", Word)]
                -- , embed _ $ sub @( '("top", Word) : env)
                ]
        , update = \() -> fromUnit <$> modify (+ 1)
        , initialAction = ()
        }

sub :: (EnvHas "top" Word env) => Component "sub" Word (Maybe Word) env
sub =
    Component
        { initialState = 0
